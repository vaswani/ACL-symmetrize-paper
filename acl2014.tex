...
\maketitle
\begin{abstract}
We present a simple approach that encourages jointly trained conditional models to agree on their parameter values. 
In contrast to methods that encourage agreement on inferences, our method can be applied even without parallel data. 
We demonstrate that parameter agreement training leads to state-of-the-art F-scores on word alignment, and significantly improves error rates on a back-transliteration task with no parallel data.
\end{abstract}
