...
Combining with alignments from Liang et al.'s model brings the total improvement to +0.8--1.5 \bleu
...
\section{Transliteration Without Parallel Text}
...
\section{Conclusion}`
We presented Parameter Agreement Training (PAT), an unsupervised method for jointly training two bidirectional alignment models that encourages agreement on parameter values and does not depend on the availability of parallel data.
...
We developed an efficient, theoretically sound optimization algorithm that falls under the EM framework. We demonstrated our formulation's effectiveness on two distinct alignment tasks. On word alignment, PAT yielded \bleu improvements comparable to the state-of-the-art ABA word alignment method, and orthogonal in the sense that combining the two methods yielded further improvements. On an English-to-Japanese back-transliteration task without parallel data (where ABA is not applicable), we obtained large reductions in error rate compared to the previous best approach.
