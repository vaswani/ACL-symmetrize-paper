...
We then constructed the inverse pipeline, required by our method PAT.
...
On top of the English-to-Japanese pipeline, implementing PAT requires a pipeline in the reverse direction - a Japanese-to-English transliteration pipeline with similar FST models.
...
The remaining required FSTs were obtained by inverting the baseline FSTs (that is, FSTs 2,3,4) and the cascase was composed in the reverse order (as depicted at the bottom of figure \ref{fig:fsts}).
In particular, the Japanese-to-English phoneme mapping FST $t_2$ was obtained by inverting $t_1$.
...
EM training of the baseline was done using the Carmel finite-state toolkit \cite{graehl97}.
...
Similarly, the PAT objective (\eqn{eqn:joint}) was maximized with respect to the phoneme mapping models $(\TA, \TB) = (t_1, t_2)$, with coefficient $\lambda\in\{1,2,3,4\}$ and while holding all other FSTs fixed.
We manipulated Carmel to compute a single E-step and output the expected counts which we then used to formulate the M-step objective. 
The M-step was solved using our own projected gradient descent implementation (A python implementation of a general purpose PGD solver for convex functions over convex closed sets. to be released).

Finally, to compare against the ordinary parallel data setting, we used Carmel to train the English-to-Japanese phoneme mapping model $t_1$ over 4.2K pairs of English and Japanese phoneme sequences. Training of all models was limited to 15 EM iterations.

Back-transliterations were obtained using regular Viterbi decoding. Note that symmetrization heuristics are not applicable, since we have no parallel data.
...
\begin{tabular}{ccc}
 ...
 &  & \tabularnewline
\end{tabular}
\caption{The 1-1 submatrix of the $t_1$ transliteration table. \textbf{Left}: Independent training. \textbf{Right}: our method (PAT); PAT learns sparser, peaked models.}
\label{fig:mapping}
\end{center}
\end{figure*}
