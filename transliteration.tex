...
We then constructed the reverse Japanese-to-English pipeline, required for PAT.
...
They train the FST cascade in both the parallel and non-parallel data settings, obtaining 40\% WNER with parallel data (trained over 3.3K word pairs), compared to 73\% WNER without parallel data (trained over 9.5K Japanese words only).
...
For training data we take the top 50\% most frequent terms from each LM, resulting in a set of 20K English terms and a set of 22.5K Japanese terms.
...
The M-step was solved using our own PGD implementation\footnote{A general purpose PGD solver for convex functions over convex closed sets. Will be released}.
...
\begin{tabular}{ccc}
 ...
 &  & \tabularnewline
\end{tabular}
\caption{The 1-to-1 mapping submatrix of the $t_1$ transliteration table. \textbf{Left}: Independent training. \textbf{Right}: PAT; PAT learns sparser, peaked models.}
\label{fig:mapping}
\end{center}
\end{figure*}
We compiled our own list of 100 US statesmen names (first and last) to be used as a test set. 
...
Using PAT we were able to significantly reduce the decipherment error rate, almost halving the gap between the models trained in the ordinary parallel data and non-parallel settings.
