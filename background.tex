...
In the non-parallel data setting, only the target entity $\vf$ is observed and the source entity $\ve$ is hidden. 
...
\end{equation}
I.e., the sequence $\vf$ can arise from any sequence $\ve$ by first selecting $\ve \sim p(\ve)$ and then proceeding according to the parallel data generative story.
...
Although the structure of $\Theta$ is unspecified, in practice, most models that follow these generative stories contain a source-to-target entity translation table (``t-table"") denoted $t$ with each parameter $t(f\mid e)$ representing the conditional probability of mapping a given source entity $e$ to target entity~$f$. 
