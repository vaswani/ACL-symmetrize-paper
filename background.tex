...
\end{equation}
That is, the observed entity $\vf$ can arise from any source sequence $\ve$ by first selecting $\ve \sim p(\ve)$ and then proceeding according to the parallel data generative story.

Unsupervised training of such models entails maximizing the data-likelihood $L(\Theta \mid X)$:
\begin{align*}
\arg\max_{\Theta}\,\,L(\Theta \mid X)
