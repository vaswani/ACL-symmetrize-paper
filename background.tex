...
We are concerned with learning generative models that describe transformations of a source-language entity $\ve$ to a target-language entity $\vf$.
We consider two different settings for these models.

In the parallel data setting, both source and target entities are observed and the generative story describes how $\vf$ can arise from $\ve$.
Denoting $\vx = (\ve, \vf)$ the model takes the following form:
\begin{equation}
p(\vx; \Theta) = p(\vf \mid \ve) = \sum_a p(\va,\,\vf \mid \ve),
\label{eqn:parallel}
\end{equation}
where $\Theta$ denotes the parameters of the model and $\va$ denotes a hidden variable that corresponds to unknown choices taken during the generative process (in our case, $\va$ is an alignment, see next subsection).
...
\end{equation}
According to the generative story that follows from this equation, the observed entity $\vf$ can arise from any source entity $\ve$ by first selecting $\ve \sim p(\ve)$ and then proceeding according to the parallel data generative story.
...
\end{align*}
Where $X=\set{(\ve_{n},\, \vf_{n})}_{n=1}^{N}$ in the parallel data setting and $X=\{\vf_{n}\}_{n=1}^{N}$ in the decipherment setting.

Next, we make these generative stories concrete by looking at the definition of $p(\va, \vf \mid \ve)$ and $p(\ve)$ (if necessary) in two sequence alignment tasks - word alignment and back-transliteration decipherment.
...
The generative story for the transformation of $\ve$ to $\vf$ follows according to \eqn{eqn:parallel} where the hidden variable $\va = (\va_1, \ldots, \va_J)$ corresponds to a sequence of $J$ indices and encodes an asymmetric target-to-source alignment under which the $j$th target word is aligned to the $a_j$th source word. 
...
Both these models assign the following conditional probability for a fixed sentence pair $(\ve, \vf)$ and a fixed alignment $\va$:
\begin{equation}
...
The exact details behind the HMM distortion parameters are implementation specific and therefore beyond the scope of this paper. See \newcite{och+ney:2003} for details of the GIZA++ implementation and \newcite{liang+:2006:align} for those of the ABA implementation.
...
\end{enumerate}
Each of their models is encoded as an FST that can be constructed and trained independently. Knight and Grael estimate FSTs 1,3,4 over monolingual data only, whereas the phoneme mapping model $p(\vj \mid \ve)$ is trained over a parallel phoneme corpus $\set{(\ve_{n},\, \vj_{n})}$ and further restricted such that each English phoneme is allowed to map to either one or two Japanese phonemes only.
...
p(\vk)=p(\vj)
=&\sum_\vw p(\vj , \vw)\\
=&\sum_\vw p(\vw) \cdot p(\vj \mid \ve)\\
=&\sum_\ve p(\ve) \sum_\va  p(\va,  \vj \mid \ve)
\end{align*}
Where $\va=((\va_{1}^{\tstart}, \va_{1}^{\tend}),\ldots,(\va_{I}^{\tstart}, \va_{I}^{\tend}))$ denotes a source-to-target monotone alignment sequence
