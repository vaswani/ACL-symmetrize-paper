...
In the E-step, each model $\Tk$ ($k\in\set{1,2}$) is held fixed and its posterior distribution over the missing data $\vz_k^n$\marginpar{DC: z appears out of the blue \bluetext{TL: better now?}} is computed per each observation, $\vx_k^n$:
$$q_k(\vz\kn, \vx\kn) \mathrel{:=} \pk(\vz\kn \mid \vx\kn ; \Tk)$$
%\textbf{parallel data}: q_k(\va \mid \ve, \vf) :=& \pA(\va \mid \ve, \vf; \TA) \\
%\textbf{decipherment}: q_1(\va, \ve \mid \vf) :=& \pA(\va, \ve \mid \vf; \TA)
%\\----\\
