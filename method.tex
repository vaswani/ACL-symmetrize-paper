...
\begin{align}
R(t_{1},t_{2})&=Tr[\sqrt{T_1}\sqrt{T_2}]\nonumber\\
&=\sum_{e}\sum_{f}\sqrt{t_{1}(f\mid e) \; t_{2}(e\mid f)}
\label{eqn:R}
\end{align}
Note that $R$ is also maximized when $\sqrt{T_1}\sqrt{T_2}=I$.
...
In the parallel data setting, we observe $N$ sequences pair $\set{\vx^n_1}_{n}=\set{(\ve^n, \vf^n)}_{n}$ or, equivalently, $\set{\vx^n_2}_{n}=\set{(\vf^n, \ve^n)}_{n}$.

In the non-parallel setting, two monolingual datasets are observed -- $N_1$ source sequences $\set{\vx^n_1}_{n}=\set{\vf^n}_{n}$ and $N_2$ target sequences $\set{\vx^n_2}_{n}=\set{\ve^n}_{n}$.
...
\begin{align}
\arg\max_{\tA, \tB}\sum_{e,f}  \Ch_1^{e,f}\log \tA(f \mid e) +\nonumber \\
\sum_{e,f} \Ch_2^{e,f} \log \tB(e \mid f) +
\lambda R(\tA, \tB)
\label{eqn:Mobj}
\end{align}
