...
Therefore, one desireable form of model agreement is in parameter
...
step, the models $P_{i}$ (for $i\in\{0,1\}$) are updated according
to the gradient of the regularized objective function $\frac{\partial}{\partial P_{i}}(L(P_{i}\mid X)+\lambda R(P_{i},P_{1-i})$
and projected back onto the probability simplex. We used simple stopping
conditions based on objective function convergence or a fixed number
of iterations. Since both regularizers are concave on the probability
simplex, our M-step is an efficiently solvable maximization program.

Finally, the gradients of the negative regularizers reveal that eaceh
regularizer encourage agreement in different manners - $R_{GM}$ is
multiplicative and $R_{CD}$ is additive:


\begin{align*}
\frac{\partial R_{GM}}{\partial P_{i}}=\frac{1}{2}\sqrt{\frac{P_{1-i}}{P_{i}}} & \text{and} & \frac{\partial R_{CD}}{\partial P_{i}}=-(P_{i}-P_{1-i}^{T})
\end{align*}
Where arithmetic operations are performed entry-wise.