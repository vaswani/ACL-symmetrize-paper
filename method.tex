...
&\ge (\alpha \sqrt{x_0 y_0} + (1-\alpha) \sqrt{x_1 y_1})^2 \\
...
with the inequality following from the arithmetic mean-geometric mean inequality.
...
In practice only the $t$ parameters are coupled through $R$,  be 
we implemented a projected gradient descent algorithm where at each step, the current $\TA, \TB$ parameters are updated according to the gradient of the objective, and then projected back onto the probability simplex.
