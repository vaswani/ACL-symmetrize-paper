...
The resulting matrix $T$ is a stochastic square matrix of dimension $|V_1|\times|V_1|$ where $|V_1|$ denotes the size of the source-language vocabulary.
...
\begin{equation*}
\trace[T]=\sum_i T_{ii}=\sum_{e}\sum_f t_{1}(f\mid e)\; t_{2}(e\mid f).
\end{equation*}
We further note that~$\trace[T]=\trace[T_{1}T_{2}]=\trace[T_{2}T_{1}]$, so that the trace captures equally well how much the target symbols map onto themselves.
...
\begin{align}
R(t_{1},t_{2})&=\trace\left[\sqrt{T_1}\sqrt{T_2}\right]\nonumber\\
&=\sum_{e}\sum_{f}\sqrt{t_{1}(f\mid e) \; t_{2}(e\mid f)}.
\label{eqn:R}
\end{align}
...
In the parallel data setting, we observe $N$ sequence pairs $\set{\vx^n_1}_{n}=\set{(\ve^n, \vf^n)}_{n}$ or, equivalently, $\set{\vx^n_2}_{n}=\set{(\vf^n, \ve^n)}_{n}$.
...
The probability of the $n$th sample under the $k$th model $\Tk$ (for $k\in\set{1,2}$) is denoted $\pk(\vx_k^n; \Tk)$.
Specifically, in the parallel data setting, the probability of $\vx_k^n$ under its model is:\footnote{This slight notational abuse helps represent both data scenarios succinctly.}\marginpar{DC: I don't even see where the notational abuse is.}
\begin{align*}
...
\label{eqn:joint}
\end{equation}
where $\lambda\ge0$ is a tunable hyperparameter (note that, in the parallel case, $N=N_1=N_2$).

We defer discussion on the relationship and merits of our approach with respect to \method{ABA} \cite{liang+:2006:align} to Section \ref{sec:related}.
...
In the E-step, each model $\Tk$ (for $k\in\set{1,2}$) is held fixed and its posterior distribution over the missing data $\vz_k^n$
...
Operationally, for models $\Tk$ that can be encoded as wFSTs (such as the IBM1, IBM2 and HMM word alignment models), the E-step
%posteriors $q_k$
...
In the M-step, we maximize the following objective with respect to $t_1$ and  $t_2$:%I\marginpar{DC: This seems exactly the same as the previous equation -- why the new notation?}
\begin{align}
\argmax_{\tA, \tB} &\sum_{e,f}  \Ch_1^{e,f}\log \tA(f \mid e) + {}\nonumber \\
&\sum_{e,f} \Ch_2^{e,f} \log \tB(e \mid f) + \lambda R(\tA, \tB)
\label{eqn:Mobj}
\end{align}
...
we applied Projected Gradient Descent \cite{bertsekas1999nonlinear,schoenemann:2011conll}, where at each step, the parameters are updated in the direction of the M-step objective gradient at $(\tA, \tB)$ and then projected back onto the probability simplex.
