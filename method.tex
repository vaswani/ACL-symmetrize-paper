...
Therefore, we modify the trace regularizer to a similar regularizer which is concave:
\begin{equation}
\label{eqn:R}
R(t_{1},t_{2})=\sum_{e,f}\sqrt{t_{1}(f\mid e) \; t_{2}(e\mid f)}
\end{equation}
Concavity of $R$ in both $t_1, t_2$ follows by observing that it is a sum of concave functions -- each term in the summation is a geometric mean, which is concave in its parameters.
...
In the non-parallel setting, two datasets are observed $\set{\vx^n_1}_{n=1}^{N_1}=\set{\ve^n}_{n=1}^{N_1}$ and $\set{\vx^n_2}_{n=1}^{N_2}=\set{\vf^n}_{n=1}^{N_2}$.
...
Using the definitions above and the \method{MIR} regularizer $R$ (Eq. \ref{eqn:R}), we formulate an optimization program for maximizing the regularized log-likelihoods $\{{L_k}\}_{k=1,2}$ of the observed data:\marginpar{DC: The use of symbols $t$ and $\Theta$ for nearly the same thing can be confusing.}
\begin{equation}
...
Using our concave regularizer, \method{MIR} optimization (\eqn{eqn:joint}) neatly falls under the MAP-EM framework \cite{Dempster:1977,??}\marginpar{insert reference to MAP-EM} and inherits the convergence properties of the underlying algorithms.
%
In MAP-EM, there is an E step and an M step as in standard EM. The E step is identical to the standard E step; the M step maximizes the complete-data log-likelihood plus the regularization term.
%
...
Specifically, in the E-step, each model $\Tk$, $k\in\set{1,2}$ is held fixed and its posterior distribution over the missing data $\vz_k^n$\marginpar{DC: z appears out of the blue} is computed per each observation, $\vx_k^n$:
...
\lambda R(t_1,t_2)
+
\sum_{k\in\set{1,2}} \sum_n  q_k(\vz\kn, \vx\kn) \log p_k(\vx\kn, \vz\kn)
\end{align*}
where $n$ ranges over the appropriate sample set.\marginpar{DC: two different $n$'s can be misleading}

Operationally, for models $\Tk$ that can be encoded as wFSTs (such as the IBM and HMM word alignment models), the E-step posteriors $q_k$ can be carried out efficiently and exactly using dynamic programming \cite{Eisner02}. 
Other models resort to approximation techniques -- for example, the fertility-based word alignment models apply hill-climbing and sampling heuristics in order to efficiently estimate the posteriors [REFERENCE -- it's not Brown et al 1993?].
...
In the M-step, we maximize the following with respect to $t_1$ and  $t_2$:\marginpar{This seems exactly the same as the previous equation -- why the new notation?}
\begin{align}
...
\marginpar{cite Schoenemann, Vaswani?}
