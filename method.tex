...
In the non-parallel data setting, we observe two sets of data points $\set{\vx^n_1}_{n=1}^{N_1} = \set{\ve_n}_{n=1}^{N_1}$ and $\set{\vx^n_2}_{n=2}^{N_2} = \set{\vf_n}_{n=1}^{N_2}$.
...
Thus, the objective in \eqn{eqn:joint} balances between similarity in parameter magnitude and data likelihood. \marginpar{\bluetext{TL: this seems like a good place to say something about invertibility}}

Another appealing property of $R$ is its concavity, which follows since $R$ is the sum of concave functions $h(x, y)=\sqrt{xy}$ over a closed convex set (We prove the concavity of $h$ in the appendix).
