Most models $\Theta$ corresponding to the generative stories described in the previous section contain some form of a source-to-target entity translation table (or simply, t-table), denoted $t$.
Each parameter $t(f\mid e)$ in the t-table represents the conditional probability of mapping given source entity $e$ to target entity $f$.

In the next subsection we suggest to jointly train two such models, a source-to-target model $\TA$ and a target-to-source model $\TB$ by coupling them through their parameters. Here, we formulate the model coupling term, which relies only on their respective t-tables $\tA, \tB$ and encodes the intuitions that the back-translation of a translation should be itself.

Specifically, let matrices $T_1, T_2$ denote the t-tables $\tA, \tB$ in matrix form and consider their multiplication $T=T_1 T_2$. 
...
Each entry $(i,j)$ in $T$ represents the total probability mass of mapping source entity $e_{i}$ to source entity $e_{j}$ by first applying the source-to-target mapping and then applying the target-to-source mapping.

In particular, each entry on the diagonal holds the probability of mapping a source entity back to itself. We therefore first consider the trace of $T$:
$$Tr[T]=\sum_i T_{ii}=\sum_{e,f}t_{1}(f\mid e)t_{2}(e\mid f)$$
Also note that $Tr[T]=Tr[T_{1}T_{2}]=Tr[T_{2}T_{1}]$.

Unfortunately, the trace is not concave in both $T_1$ and $T_2$, a property which will become desirable in optimization. Therefore, we propose a similar term to trace:
$$R(T_{1},T_{2})=\sum_{e,f}\sqrt{t_{1}(f\mid e)t_{2}(e\mid f)}$$

Now, concavity of $R$ in both $T_1, T_2$ follows from the fact that it is a sum of concave functions (each term in the sum is a geometric mean of two parameters and it is well known that the geometric mean is concave, and that the sum of concave functions is concave).
