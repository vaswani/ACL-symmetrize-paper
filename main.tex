...
(as opposed to through their inferences, as in Liang et al.'s Alignment by Agreement (\method{ABA})), our method seamlessly extends to all IBM-style word alignment models as well as to sequence alignment \emph{without} parallel data.
Our proposed algorithm is mathematically sound and inherits convergence guarantees from EM.

We evaluated \method{MIR} on two tasks:
(1)~On word alignment, applying \method{MIR} on fertility based models we attain higher F-scores than \method{ABA}. Interestingly, combining the alignments produced by both methods significantly outperform either method alone on \bleu.
...
\section{Transliteration\,Decipherment\,Experiment}
