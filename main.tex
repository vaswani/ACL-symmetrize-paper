...
\maketitle
\begin{abstract}
We present Model Invertibility Regularization (\method{MIR}), a method for jointly training bi-directional sequence alignment models that takes into account the invertibility of the alignment task.
By coupling the two alignment models through their parameters (as opposed to through their inferences, as in Liang et al.'s Alignment by Agreement (\method{ABA})), our method remains applicable to all IBM-style word alignment models as well as to sequence alignment \emph{without} parallel data.
Our proposed algorithm is mathematically sound and inherits the convergence guarantees of EM.
...
Interestingly, the two methods produce complementary alignments, which when combined, significantly outperform either method in \bleu.
