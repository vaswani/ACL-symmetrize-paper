...
\author{Tomer Levinboim$^\ast$ \\
  %University of Notre Dame\\
  %Department of Computer Science\\
  {\tt levinboim.1@nd.edu} \And
  Ashish Vaswani$^\dag$ \\
  %University of Southern California \\
  %Information Sciences Institute \\
  {\tt avaswani@isi.edu} \And
  David Chiang$^\ast$ \\
  %University of Notre Dame\\
  %Department of Computer Science\\
  {\tt dchiang@nd.edu}
\AND
\normalfont $^\ast$Dept. of Computer Science and Engineering \\
University of Notre Dame \And
\normalfont $^\dag$Information Sciences Institute \\
University of Southern California
}
\date{}
...
We present Model Invertibility Regularization (\method{MIR}), a method that jointly trains two directional sequence alignment models, one in each direction, and takes into account the invertibility of the alignment task.
By coupling the two models through their parameters
(as opposed to through their inferences, as in Liang et al.'s Alignment by Agreement (\method{ABA}), and Ganchev et al.'s Posterior Regularization (\method{PostCAT})), our method seamlessly extends to all IBM-style word alignment models as well as to alignment \emph{without} parallel data.
Our proposed algorithm is mathematically sound and inherits convergence guarantees from EM.
We evaluate \method{MIR} on two tasks:
(1)~On word alignment, applying \method{MIR} on fertility based models we attain higher F-scores than \method{ABA} and \method{PostCAT}.
%Interestingly, combining the alignments produced by both methods significantly outperform either method alone on \bleu.
(2)~On Japanese-to-English back-transliteration without parallel data, applied to the decipherment model of Ravi and Knight, \method{MIR} learns sparser models that close the gap in whole-name error rate by 33\% relative to a model trained on parallel data, and further, beats a previous approach by Mylonakis et al.
...
\section{Experiments with Parallel Data}
\label{sec:alignment}
\input{alignment.tex}

\section{Experiments without Parallel Data}
\label{sec:transliteration}
\input{transliteration.tex}

\section{Conclusion}
We presented Model Invertibility Regularization (\method{MIR}), an unsupervised method for jointly training bidirectional sequence alignment models with or without parallel data.
Our formulation is based on the simple observation that the alignment tasks at hand are inherently invertible and encourages the translation tables in both models to behave like pseudo-inverses of each other.

We derived an efficient MAP-EM algorithm and demonstrated our method's effectiveness on two different alignment tasks. On word alignment, applying \method{MIR} on IBM4 model yielded the highest F scores and the resulting \bleu scores were comparable to that of Alignment by Agreement \cite{liang+:2006:align} and \method{PostCAT} \cite{ganchev-gracca-taskar:2008}.
%and complementary, so that combining the produced alignments yielded further improvement (up to 1.5 \bleu).
Our best results (up to +1.5 \bleu improvement) were obtained by combining alignments from both \method{MIR} and \method{ABA}, indicating that the two methods learn complementary alignments.
On Japanese-English back-transliteration with no parallel data, we obtained a significant error reduction over two baseline methods \cite{RK09,Mylonakis2007}.

As future work, we plan to apply \method{MIR} on large-scale MT decipherment \cite{Ravi11,Dou13}, where, so far, only a single directional model has been used.
Another promising direction for \method{MIR} is to encourage invertibility not only between words, but between their senses and synonyms.
%\section*{Appendix}
...
We would like to thank Markos Mylonakis and Khalil Sima'an for their help in understanding the derivation of their \method{bi\mbox{-}EM} method. This work was partially supported by DARPA grants DOI/NBC D12AP00225 and HR0011-12-C-0014 and a Google Faculty Research Award to Chiang.
