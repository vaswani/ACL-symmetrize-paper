...
\subsection{Parallel Data Baseline: $\method{ABA}$ and $\method{PostCAT}$}
Our approach is most similar to Alignment by Agreement \cite{liang+:2006:align} which uses a single joint objective for two word alignment models.
...
$\method{MIR}$ regularization has both theoretical and practical advantages compared to $\method{ABA}$, which make our method more convenient and broadly applicable:
...
Thus, even if we somehow adapt $\method{ABA}$'s product-of-marginals heuristic to such models, we run the risk of estimating highly inaccurate posteriors (specifically, zero-valued posteriors).
...
\newcite{ganchev-gracca-taskar:2008} propose \method{PostCAT} which uses Posterior Regularization \cite{ganchev2010posterior} to enforce posterior agreement between the two models.
Specifically, they add a KL-projection step after the E-step of the EM algorithm which returns the posterior $q(z\mid x)$ closest in KL-Divergence to ordinary E-step posterior which also upholds certain constraints. 
The particular constraints they suggest encode alignment agreement \emph{in expectation} between the two model' posteriors. For details, the reader can refer to \cite{ganchev-gracca-taskar:2008}. 

Similarly to \method{ABA}, with their suggested alignment agreement constraints $\method{PostCAT}$ cannot be applied without parallel data and it is unclear how to extend it to fertility based models (however, it does seems possible to apply other constraints using the general posterior regularization framework).

We compare \method{MIR} against \method{ABA} and \method{PostCAT} in Section \ref{sec:alignment}.
...
&=\log \prod_n \sum_\ve p(\ve) \prod_{m} t_1(f^n_m\mid\ve)
\end{align*}
...
To solve for $\beta$ in \eqn{eqn:biEM}, \newcite{Mylonakis2007} propose \method{bi\mbox{-}EM}, an iterative EM-style algorithm.
%in which both E- and M- steps have a closed-form solution. They do not provide the algorithm derivation.
%we observe that the solution they proposed for the M-step does not follow from their M-step objective.
The objective function in their M-step is
%Inspecting their M-step objective, we determined that it is
not concave, hinting that a closed-form solution for the maximizer is unlikely.
The probability estimate that they use in the M-step appears to maximize
an approximation of their M-step objective which omits the normalization factors in \eqn{eqn:biEM}.
\marginpar{DC: Will motivated readers be able to understand what ``omit normalization factors'' means? TL: added reference to Eq.}
...
Nevertheless, \method{bi\mbox{-}EM} attains improved results compared to standard EM on both POS-tagging and monotone noun sequence translation without parallel data. We compare \method{MIR} against \method{bi\mbox{-}EM} in Sec. \ref{sec:transliteration}.
