...
Thus, even if we somehow adapt $\method{ABA}$'s product-of-marginals heuristic to such models, we run the risk of estimating highly inaccurate posteriors (specifically, 0 valued posteriors).
...
Inspecting their M-step objective we determined that it is not concave, hinting that a closed-form solution for the maximizer is unlikely.
In fact, their proposed solution for the M-step can be obtained from an approximation of their M-step objective in which the normalization factors were omitted.
...
Despite its heuristic M-step, \method{bi\mbox{-}EM} attains improved results compared to standard EM on both POS-tagging and monotone noun sequence translation without parallel data. 

We compare \method{MIR} against \method{bi\mbox{-}EM} in Section \ref{sec:transliteration}.
