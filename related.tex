...
\label{eqn:reparam}
\end{equation}
They then maximize the likelihood of \emph{monolingual} sequences from both languages:
\begin{equation}
...
\end{align*}
where $p(\ve)$ denotes the probability of $\ve$ according to a fixed source language model.
...
Inspecting their M-step objective and its proposed solution, we determined that (1) the objective is not concave, hinting that a closed-form solution for the maximizer is unlikely, and (2) the proposed solution is the maximizer of a different objective function, obtained by simply omitting the normalization denominators (See \eqn{eqn:reparam}) from their M-step objective.
