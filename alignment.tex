...
For $\method{ABA}$\footnote{\url{http://cs.stanford.edu/~pliang/software/cross-em-aligner-1.3.zip}}
 and $\method{PostCAT}$\footnote{\url{http://www.seas.upenn.edu/~strctlrn/CAT/CAT.html}} 
 training we used the authors' implementation, which supports the HMM model.
...
Our method $\method{MIR}$ was implemented on top of GIZA++.\footnote{\url{https://github.com/vaswani/MIR_ALIGNMENT}}



%\marginpar{DC: Are you ready to release? Can you provide a URL?}
...
\hline
EM-HMM & 23.6 & 16.7 & 17.1\tabularnewline
\method{MIR}-HMM & 24.0 & 17.1 & 17.6 \tabularnewline
\method{ABA}-HMM & 24.4 & 17.1  & 17.7 \tabularnewline
\method{PostCAT}-HMM & 24.6 & 16.9  & 17.4\tabularnewline
\hline
\method{EM}-IBM4 & 24.2 & 16.8& 17.2\tabularnewline
\method{MIR}-IBM4 & 24.6& 17.2& 17.5\tabularnewline
\hline
\method{ABA\&MIR}-HMM & 25.1 & 17.4 & 17.9
\label{tab:resultsB}
\end{tabular}}
\caption{\bleu scores. $\method{MIR}$, $\method{ABA}$  obtain statistically significant gains over standard EM training. Combining $\method{ABA}$ and $\method{MIR}$ alignments improves \bleu{} score significantly over $\method{ABA\mbox{/}MIR}$ ($p<0.05$). All \bleu differences between $\method{ABA}$ and $\method{MIR}$ are not statistically significant.}
\end{table}
