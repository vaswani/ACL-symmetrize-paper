...
\end{tabular}
\end{center}
\caption{For Czech-English, the year refers to the WMT shared task; for Chinese-English, the year refers to the NIST Open MT Evaluation. Both ABA and PAT obtain statistically significant results over vanilla EM training, and combining ABA and PAT alignments also improves \bleu{} score significantly over ABA, with $p<0.05$ for for these improvements. The \bleu{} difference between ABA and PAT is not statistically significant.  }
\label{tab:results}
\end{table*}
...
On both language pairs, PAT performs significantly better than vanilla EM training, and compares favorably with ABA. The \bleu{} difference between PAT and ABA for Chinese-English and Czech is not statistically significant according to a bootstrap resampling significance test~\cite{koehn-stat:2004}. However, the two approaches are not doing the same thing, as can be seen by combining the alignments (simply concatenating two copies of the parallel text with the two alignments): on Chinese-English, the total improvements reaches +1.5 \bleu{}.
