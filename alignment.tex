...
On both language pairs, PAT performs significantly better than vanilla EM training, and compares favorably with ABA. The \bleu{} difference between PAT and ABA for Chinese-English is not statistically significant according to a bootstrap resampling significance test~\cite{koehn-stat:2004}.\marginpar{DC: What about the other differences, are they significant?} 
However, the two approaches are not doing the same thing, as can be seen by combining the alignments (simply concatenating two copies of the parallel text with the two alignments): on Chinese-English, the total improvements reaches +1.5 \bleu{}.
