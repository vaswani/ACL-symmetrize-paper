...
We carried out Czech-English and Chinese-English word alignment and translation experiments comparing $\method{MIR}$ against standard EM training and $\method{ABA}$.
...
Vanilla EM training was done using the popular word alignment toolkit  GIZA++\footnote{code.google.com/p/giza-pp/} which supports advanced IBM models as well as the HMM model. 
We implemented $\method{MIR}$ on top of GIZA++ and plan to release our toolkit as a patch over GIZA++, for practitioners to use in their translation experiments. 
...
%\textbf{Task} & \textbf{Data(tokens)}& \textbf{System} & \textbf{Align F1} (\%)  &\multicolumn{3}{c}{\textbf{\bleu} (\%)} \\
%     &      &        &             & 2008 & 2009 & 2010 \\
%\hline
%\multirow{3}{*}{Chi / Eng} & \multirow{3}{*}{5.3M / 6.6M} & EM-HMM & 64.6&  23.6& & \\
%        &       & $\method{MIR}$-HMM &  70.9 (+6.3) & 24.0 (+0.4) & &\\
...
%
%\hline
%\multirow{3}{*}{Cze / Eng} & \multirow{3}{*}{1.6M / 1.8M} & HMM-EM & 65.0 & & 16.7 & 17.1  \\
%        &       & $\method{MIR}$-HMM   & 69.6 (+4.6)& & 17.1 (+0.4)& 17.6 (+0.5)& \\
...
\caption{For Czech-English, the year refers to the WMT shared task; for Chinese-English, the year refers to the NIST Open MT Evaluation. Both $\method{ABA}$ and $\method{MIR}$ obtain statistically significant results over vanilla EM training. Combining $\method{ABA}$ and $\method{MIR}$ alignments also improves \bleu{} score significantly over $\method{ABA\mbox{/}MIR}$, with $p<0.05$ for these improvements. The \bleu{} difference between $\method{ABA}$ and $\method{MIR}$ is not statistically significant.}
\label{tab:results}
\end{table*}


\subsection{Experiments}
We first trained the word alignment models on the following parallel data (with sentence length restricted to at most $40$ tokens):
\begin{itemize}
\item \textbf{Chinese-English}: data from the NIST 2009 Open MT Evaluation constrained task.
\item \textbf{Czech-English}: data from the News Commentary corpus.
\end{itemize}
...
For IBM model 4, we applied the standard grow-diag-final-and (gdfa) symmetrizaition heuristic (REFERENCE).
...
In particular, for $\method{MIR}$ we found $\lambda=300$ to work best on both datasets.
