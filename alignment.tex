...
For all HMM models, we used the \emph{posterior decoding} technique from \cite{liang+:2006:align} as used by the $\method{ABA}$ implementation.
...
Table~\ref{tab:resultsF} summarizes our alignment F-scores. 
Using $\method{MIR}$ we gained modest improvement on Czech-English (the smaller task) and almost doubled 

\begin{table}[h]
\begin{tabular}{r|l|l}
 & \multicolumn{1}{c|}{Chinese-English } & \multicolumn{1}{c}{Czech-English}\tabularnewline
Method & \textbf{Align F1} & \textbf{Align F1}\tabularnewline
\hline 
EM-HMM & 64.6 & 65.0\tabularnewline
MIR-HMM & 70.9 (+6.3) & 69.6 (+4.6)\tabularnewline
ABA-HMM & 70.8 (+6.2) & 70.4 (+5.4)\tabularnewline
\hline 
EM-M4 & 68.4 & 67.3\tabularnewline
MIR-M4 & 72.9 & 70.7 (+4.6)\tabularnewline
\end{tabular}
%\end{center}
...
We tested the effect of alignments obtained by $\method{MIR}$ on translation quality, using the  Moses~\cite{koehn2007moses} phrase-based translation system\footnote{http://www.statmt.org/moses/}.
...
MIR-HMM & 24.0 (+0.4) & 17.1 (+0.4) & 17.6 (+0.5)\tabularnewline
ABA-HMM & 24.4 (+0.8) & 17.1 (+0.4) & 17.7 (+0.6)\tabularnewline
combined & 25.1 (+1.5) & 17.4 (+0.7) & 17.9 (+0.8)\tabularnewline
\end{tabular}}
\caption{\bleu scores using only the HMM models. Both $\method{ABA}$ and $\method{MIR}$ obtain statistically significant results over standard EM training. Combining $\method{ABA}$ and $\method{MIR}$ alignments improves \bleu{} score significantly over $\method{ABA\mbox{/}MIR}$, with $p<0.05$ for these improvements. The \bleu{} difference between $\method{ABA}$ and $\method{MIR}$ is not statistically significant.}
\label{tab:resultsB}
\end{table}
