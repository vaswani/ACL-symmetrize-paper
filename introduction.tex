...
One typical generation event is entity-to-entity translation, parameterized by a translation table $t(\ve \mid \vf)$ that defines the probability of mapping a single source entity $\ve$ to a single target entity $\vf$ (e.g., the IBM model translation table \cite{brown+alii:1993}).
...
Considering that the underlying task (translation, transliteration) is invertible, 
...
Specifically, letting $t_1$ and $t_2$ denote the two models' entity-translation tables, we would expect that $t_1(\vf \mid \ve)$ is high if and only if $t_2(\ve \mid \vf)$ is high. However, this is typically not the case.
