...
Left unchecked, independently training two such directional models (source-to-target and target-to-source) often yields two models that diverge from this invertibility intuition. In word alignment, this can lead to disagreements between the alignments inferred by a model trained in one direction and those inferred by a model trained in the reverse direction.
...
Using \method{MIR}, we are able to reduce the whole-name error-rate relative to a model trained on parallel data by 33\%, as well as significantly outperform the joint model of \newcite{Mylonakis2007}.
