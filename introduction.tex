...
On Czech-English and Chinese-English word alignment (Section \ref{sec:alignment}), restricted to the HMM model, \method{MIR} attains F- and \bleu score improvements that are quantitatively comparable to those of \method{ABA}. We further apply \method{MIR} beyond HMM, on the fertility-based IBM Model 4, showing gains in F-score compared to both the baseline IBM model 4 and \method{ABA}.


but qualitatively different, so that combining both alignments yields additive gains over each method by itself (over two of the three translation test sets).
