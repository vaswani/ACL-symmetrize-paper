...
To remedy the disparity (and other shortcomings), we usually resort to ad-hoc alignment symmetrization techniques such as grow-diag-final-and \cite{koehn+:2003} that heuristically combine alignments from both directions.

\newcite{liang+:2006:align} suggest a more fundamental approach they call ``Alignment by Agreement'' (ABA) which jointly trains the two models by maximizing their individual data-likelihoods along with a regularizer that rewards agreement between their alignment posteriors (computed over each parallel sentence pair).
...
In this paper we develop a different approach for jointly training bi-directional sequence alignment models called MIR (``Model Invertibility Regularization'').
...
(2) MIR can be applied to sequence alignment even without access to parallel data at all.
(3) The MIR algorithm faithfully follows the generalized EM recipe and so possesses theoretical convergence guarantees.

The key idea facilitating the extension to more complex models and to different data settings is in our regularizer, which regularizes the model parameters as opposed to their inferences. 
Specifically, our regularizer was designed to reward model pairs whose entity-translation tables respect our intuition regarding the invertibility of the alignment task.

We tested and compared MIR to competitive baselines on two sequence alignment tasks - word alignment and back-transliteration without parallel data (a.k.a. ``Decipherment'').
...
On English-Japanese back-transliteration decipherment we applied MIR to the cascade of FSTs approach proposed by \newcite{RK09}. 
Using MIR, we were able to reduce the whole-name error-rate relative to a model trained on parallel data by 33\%, as well as significantly outperform a more competitive jointly trained baseline by Markos et al.
