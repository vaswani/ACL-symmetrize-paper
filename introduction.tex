...
For example, the IBM models for word alignment \cite{brown+alii:1993} describe a stochastic process by which the target sentence is generated from the source sentence via reordering and word-for-word translation.

However, this directionality may come with a price. \marginpar{DC: There are other problems too, right? There is some symmetry that is not being captured by the model.} It often imposes restrictions on the inference space: in the IBM word alignment models, only one-to-many word alignments (target to source) are possible, which may not fit well for some language pairs (for example, German compound target words usually arise from multiple English source words). 
To remedy this, we often resort to ad-hoc techniques, such as grow-diag-final-and \cite{koehn+:2003}, that combine alignments in two directions.
While this resolves some of the problems, independent training can still lead the two models to divergent local maxima, whose disagreeing inferences cannot be fixed by post-processing. 

A second problem with training alignment models is dependence on parallel data.
...
For example, \newcite{KG98} manually collected thousands \marginpar{insert number} of parallel English-Japanese sequences to train their phoneme alignment model. 
...
\newcite{liang+:2006:align} suggest an approach they call Alignment by Agreement (ABA) that jointly trains the two directional models, each maximizing its own data likelihood (as would be done independently) as well as a model coupling term that encodes a measure of agreement between the models.\marginpar{DC: need to make clear from the beginning that the agreement term is defined on alignments (posteriors), not parameters.} 
Their suggested agreement measure further exploits the observed parallel training data by rewarding similarity between the two alignment posteriors (one for each direction) computed on each training sentence pair. 
Indeed, even though their optimization procedure ultimately does not optimize a clear objective function,\marginpar{DC: this criticism may need to be justified} their experimental results demonstrate the effectiveness in jointly training the two models.
...
\newcite{RK09} train English-to-Japanese transliteration models where only Japanese data is observed.\marginpar{DC: They use an English language model, though, right?; \bluetext{TL: right. fixed.}}
...
Compared to ABA, our approach operates with a different measure of agreement (Section \ref{sec:method}).
