...
For example, in transliteration, a source-language word is mapped to a target language's writing system under a sound preserving mapping (for example, ``computer'' to Japanese Romaji, ``konpyutaa''). The original word should then be easily recoverable from its transliterated version.
Similarly, in translation, the back-translation of a translation of a word is likely to be that same word itself.
...
Finally, the \method{MIR} algorithm faithfully follows the EM recipe and inherits its theoretical convergence guarantees.
...
On Czech-English and Chinese-English word alignment (Section \ref{sec:alignment}), restricted to the HMM model, \method{MIR} attains F- and \bleu score improvements that are quantitatively comparable to those of \method{ABA}. 
We further apply \method{MIR} beyond HMM, on the fertility-based IBM Model 4, showing gains in F-score compared to both the baseline IBM model 4 and \method{ABA}.
