...
In NLP however, the commonplace generative models describing such phenomena are directional, only concerning with the transfer of source-language entities to target-language entities or vice versa, but not both.
Left unchecked, independently training two such directional models (source-to-target and target-to-source) often yields two models that diverge from this invertibility intuition. Specifically, in word alignment, the alignments inferred by a model trained in one direction often mismatch those inferred by a model trained in the reverse direction.
...
their alignment experimental results demonstrate the effectiveness in jointly training bidirectional models. 

In this paper we develop a different approach for jointly training bi-directional sequence alignment models called ``Model Invertibility Regularization'', denoted $\method{MIR}$ (Section \ref{sec:method})
Our approach has several benefits over $\method{ABA}$:
(1) $\method{MIR}$ can be applied to sequence alignment even without parallel data.
(2) A single implementation extends to all IBM models, including the fertility based models.
(3) The $\method{MIR}$ algorithm faithfully follows the EM recipe and inherits its theoretical convergence guarantees.
...
Specifically, $\method{MIR}$ was designed to reward model pairs whose entity-translation tables respect the invertibility intuition.
...
On English-Japanese back-transliteration decipherment (Section \ref{sec:transliteration}) we applied $\method{MIR}$ to the cascade of wFSTs approach proposed by \newcite{RK09}.
Using $\method{MIR}$, we were able to reduce the whole-name error-rate relative to a model trained on parallel data by 33\%, as well as significantly outperform the joint model of \newcite{Mylonakis2007}.
