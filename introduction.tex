...
Specifically, they introduce a model coupling term that rewards similarity between each sentence pair's alignment posteriors.
...
Indeed, even though their optimization procedure is heuristic\marginpar{DC: this criticism may need to be justified. \bluetext{TL: changed}} their experimental results demonstrate the effectiveness in jointly training bi-directional two models.
%and show that training with posterior agreement leads to better performing models (reduced AER). 
However, the dependence on posterior agreement limits their joint alignment approach to the parallel data setting.
