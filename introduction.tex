The transfer of information between languages is a common natural language phenomenon that is intuitively invertible. 
For example, in transliteration, a source-language word is mapped to a target language's writing system under a sound preserving mapping (for example, ``computer'' to Japanese Romaji, ``konpyutaa''). The original word should then be recoverable from its transliterated version.
Similarly, in translation, the back-translation of the translation of a word is likely to be that same word itself.

In NLP, however, commonly-used generative models describing such phenomena are directional, only concerned with the transfer of source-language symbols to target-language symbols or vice versa, but not both directions.
Left unchecked, independently training two such directional models (source-to-target and target-to-source) often yields two models that diverge from this invertibility intuition. 

In word alignment, this can lead to disagreements between alignments inferred by a model trained in one direction and those inferred by a model trained in the reverse direction.
To remedy this disparity (and other shortcomings), it is common to turn to alignment symmetrization techniques such as grow-diag-final-and \cite{koehn+:2003} which heuristically combines alignments from both directions.

%------ VERSION 1

%\marginpar{DC: Consider mentioning MIR before ABA: \bluetext{TL: see commented out version 2}}
\newcite{liang+:2006:align} suggest a more fundamental approach they call Alignment by Agreement ($\method{ABA}$), which jointly trains two word alignment models by maximizing their data-likelihoods along with a regularizer that rewards agreement between their alignment posteriors (computed over each parallel sentence pair).
Although their EM-like optimization procedure is heuristic, 
%\marginpar{\bluetext{"Liang et al.: we use a heuristic approximation which nonetheless works well in practice."}} 
it proves effective at jointly training bidirectional models. \newcite{ganchev-gracca-taskar:2008} propose another approach for agreement between the directed translation models by adding constraints on the alignment posteriors. Unlike \method{ABA}, their optimization is exact, but it can be computationally expensive, requiring multiple forward-backward inferences in each E-step.
%\comment{that encourage the directed translation models to agree on their alignments.}

In this paper we develop a different approach for jointly training general bidirectional sequence alignment models called Model Invertibility Regularization, or \method{MIR} (Section \ref{sec:method}).
Our approach has two key benefits over \method{ABA} and \method{PostCAT}:
First, \method{MIR} can be applied to sequence alignment without parallel data.
Second, a single implementation seamlessly extends to all IBM models, including the fertility based models.
Furthermore, since \method{MIR} follows the MAP-EM framework, it inherits its desirable convergence guarantees.

The key idea facilitating the easy extension to complex models and to non-parallel data settings is in our regularizer, which operates on the model parameters as opposed to their inferences. 
Specifically, \method{MIR} was designed to reward model pairs whose translation tables respect the invertibility intuition.

%---- VERSION 2 (new)
%%
%In this paper we develop an approach we call Model Invertibility Regularization, or \method{MIR} (Section \ref{sec:method}). Our approach jointly trains two bidirectional sequence alignment models, while regularizing their parameters to respect the invertibility intuition.
%
%Our approach is similar to the Alignment by Agreement ($\method{ABA}$) approach of \newcite{liang+:2006:align} who suggest to maximize the two models' data-likelihoods along with a regularizer that rewards agreement between their alignment posteriors (computed over each parallel sentence pair). 
%
%\method{MIR} has several benefits over \method{ABA}:
%First, \method{MIR} can be applied to sequence alignment even without parallel data.
%Second, a single implementation seamlessly extends to all IBM models, including the fertility based models.
%Finally, the \method{MIR} algorithm faithfully follows the EM recipe and inherits its theoretical convergence guarantees. This is in contrast to \method{ABA} which resorts to an approximation heuristic.
%
%The key idea facilitating the extension to complex models and to non-parallel data settings is in our regularizer, which operates on the model parameters as opposed to their inferences. 
%
%---- END

We tested \method{MIR} against competitive baselines on two sequence alignment tasks: word alignment (with parallel data) and back-transliteration decipherment (without parallel data).

On Czech-English and Chinese-English word alignment (Section \ref{sec:alignment}), restricted to the HMM model, \method{MIR} attains F- and \bleu score improvements that are comparable to those of \method{ABA}. 
We further apply \method{MIR} beyond HMM, on the fertility-based IBM Models, showing gains in F-score compared to both the baseline IBM models and \method{ABA}.
Interestingly, the alignments obtained by \method{ABA} and \method{MIR} are qualitatively different, so that combining the two yields additive gains over each method by itself.

On English-Japanese back-transliteration decipherment (Section \ref{sec:transliteration}), we apply \method{MIR} to the cascade of wFSTs approach proposed by \newcite{RK09}.
Using \method{MIR}, we are able to reduce the whole-name error-rate relative to a model trained on parallel data by 33\%, as well as significantly outperform the joint model proposed by \newcite{Mylonakis2007}.
...
%Although their optimization procedure is inexact, their experimental results demonstrate the effectiveness of jointly training bidirectional models.
...
%one could hope that training these models in both directions (source-to-target and target-to-source) would yield two models that agree on their model parameterization  and inferred alignments.
%%Specifically, letting $t_1$ and $t_2$ denote the two models' entity-translation tables, we would expect that $t_1(\vf \mid \ve)$ is high if and only if $t_2(\ve \mid \vf)$ is high. 
%However, this is typically not the case, which leads to some problems.
%
%For one, the model's directionality imposes restrictions on the inference space: in the IBM word alignment models, each target word can align to at most one source word, which may not be appropriate for some language pairs (for example, German compound target words usually arise from multiple English source words). 
%To remedy this, we often resort to ad-hoc techniques, such as grow-diag-final-and \cite{koehn+:2003}, that heuristically combine alignments in two directions.
%While this post-processing resolves some problems, it does not address the core problem -- unsupervised independent training of each model can lead to two local maxima that diverge in their parameters and ultimately their inferences. \newcite{liang+:2006:align} suggest an approach they call Alignment by Agreement (ABA) that jointly trains the two directional models by maximizing their individual data likelihood as well as a model coupling term. Their coupling term rewards agreement between the alignment posteriors computed for each parallel sentence pair.
%Although their optimization procedure is inexact, their experimental results demonstrate the effectiveness of jointly training bidirectional models.
%However, the dependence of their coupling term on both posteriors means that their approach only works on parallel data.
%
%In this paper we present Parameter Agreement Training (PAT); a method for jointly training directional alignment models that encourages agreement on parameter values.
%In contrast to ABA, our approach can be applied in both parallel and non-parallel settings, allowing us to extend beyond word alignment, to challenging tasks such as decipherment of transliterations. % where acquiring parallel data can be challenging. 
%%Recent work has begun to explore the use of non-parallel data. 
%
%For example, \newcite{RK09} train English-to-Japanese transliteration models where only Japanese data is observed. 
%Unfortunately, the resulting model considerably underperforms compared to the same model trained on parallel data. But using PAT, we are able to reduce the whole-name error rate relative to the parallel-data trained model by 33\%.
%
%On Czech-English and Chinese-English word alignment, 
%we obtain improvements in alignment accuracy by up to 6.3 F1 points, 
%and improvements in translation accuracy of up to 0.5 \bleu, 
%both comparable to the improvements obtained by ABA. 
%Interestingly, the ABA and PAT improvements are nearly additive: 
%by combining our alignments with those from ABA, we obtain a total improvement of 0.7-1.5 \bleu.
%
%Computationally, our resulting objective function can be efficiently optimized using EM, where the E-step remains unchanged and the M-step reduces to an instance of convex programming.
%
