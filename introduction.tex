Alignment is a crucial subtask for many larger \marginpar{\bluetext{TL: large?}} NLP problems: 
...
The goal is to align entities (e.g., words, phonemes) between two sequences, for example, English and French sentences, or English phonemes and Japanese phonemes.

The most common models used in these tasks are both \emph{generative} and \emph{directed}, that is, they define a directed sequence of generation events by which one side of the pair (the source) produces the other (the target). 
The simpler generative processes usually involves an entity-to-entity translation table, denoted $t(\ve \mid \vf)$, that defines the probability of mapping a single source entity $\ve$ to a single target entity $\vf$ (for example, the IBM model translation table \cite{brown+alii:1993}).
...
Given that the underlying task (translation, transliteration) seems invertible, 
one could hope that training these models in both directions (source-to-target and target-to-source) would yield two models that agree on their inferred alignments and their model parameterization.
Specifically, letting $t_1$ and $t_2$ denote the two models' entity-translation tables, we expect that $t_1(\vf \mid \ve)$ is high if and only if $t_2(\ve \mid \vf)$ is high (intuitively, the translation of the translation is itself).

\marginpar{DC: There are other problems too, right? There is some symmetry that is not being captured by the model.\bluetext{TL: modified}}
However, such model agreement is often not the case. 
For one, the model's directionality imposes restrictions on the inference space: in the IBM word alignment models, only one-to-many word alignments (target to source) are possible, which may not fit well for some language pairs (for example, German compound target words usually arise from multiple English source words). 
To remedy this, we often resort to ad-hoc techniques, such as grow-diag-final-and \cite{koehn+:2003}, that heuristically combines alignments in two directions.
While this post-processing resolves some problems it does not address the core problem - unsupervised independent training of each model may very well lead to two local maxima that diverge on their translation parameters and ultimately their inferences.

A second fundamental problem with training alignment models is the dependence on parallel data.
In word alignment, most available parallel data is limited to small number of languages.
...
\marginpar{DC: need to make clear from the beginning that the agreement term is defined on alignments (posteriors), not parameters. \bluetext{TL: changed}} 
Specifically, they introduce a model coupling term that rewards agreement between each sentence pair's alignment posteriors.
...
Indeed, even though their optimization procedure is heuristic\marginpar{DC: this criticism may need to be justified. \bluetext{TL: changed}} their experimental results demonstrate the effectiveness in jointly training bi-directional models.
%and show that training with posterior agreement leads to better performing models (reduced AER). 
However, the dependence of their coupling term on both posteriors limits their joint alignment approach to the parallel data setting.
...
\newcite{RK09} train English-to-Japanese transliteration models where only Japanese data is observed.\marginpar{DC: They use an English language model, though, right?; \bluetext{TL: right. changed.}}
Unfortunately, the resulting model considerably underperforms compared to the same model trained on parallel data.
...
Compared to ABA, our approach (Section \ref{sec:method}) operates with a different measure of agreement; one that encourages similarity between the two models' parameterizations.
