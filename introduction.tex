...
Left unchecked, independently training two such directional models (source-to-target and target-to-source) often yields two models that diverge from this invertibility intuition. In word alignment, this can lead to disagreements between alignments inferred by a model trained in one direction and those inferred by a model trained in the reverse direction.
...
We tested \method{MIR} against competitive baselines on two sequence alignment tasks: word alignment and back-transliteration without parallel data.

On Czech-English and Chinese-English word alignment (Section \ref{sec:alignment}), restricted to the HMM model, our approach attains F- and \bleu score improvements that are quantitatively comparable to those of \method{ABA}, but qualitatively different, so that combining both alignments yields additive gains over each method by itself.
We further apply our approach to the fertility-based IBM Model 4, showing significant gains in \bleu compared to baseline model.\marginpar{DC: what are ordinary models. \bluetext{TL: changed to "baseline"}}
